...
const ArticleTable = ({ data, onEdit, onDelete, page, totalPages, handleNextPage, handlePrevPage, loading }) => {
          ...
          <>
            {data.length === 0 ? (
              <div className="text-center py-16 bg-white">
                ...
                <h3 className="text-primary-950 text-lg font-medium mb-1">No Articles found</h3>
                <p className="text-primary-950/70 mb-6">Add a new article to get started</p>
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-primary-950/5 py-5">
                    <tr className="text-left text-sm font-semibold text-primary-950 border-b border-primary-950/10">
                      ...
                      {/* <th className="py-4 pl-3 text-left font-medium tracking-wider">Author</th> */}
                  ...
                  <tbody className="bg-white divide-y divide-primary-950/10">
                    {data.map((entry, index) => (
                      <tr key={entry.id} className="hover:bg-primary-950/5 transition-colors">
                        ...
                        <td 
                          className="py-4 pl-3 text-primary-950 cursor-pointer"
                          onClick={() => openModal(entry)}
                        >
                          <div className="w-56 truncate">
                            {truncateText(entry.description, 5)}
                          </div>
                        </td>
                        {/* <td className="py-4 pl-3">
                          <a 
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  <span className="text-md py-5 font-medium text-gray-500">Title</span>
                  <p className="text-gray-800 font-medium">{selectedArticle.name}</p>
                </div>
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-gray-500">Description</p>
                <div
                  dangerouslySetInnerHTML={{ __html: sanitizeHTML(selectedArticle.description) }}
