...
const ServiceProviderTable = ( { data, onEdit, onDelete, page, totalPages, handleNextPage, handlePrevPage, loading, showActions } ) => {
          ...
          <>
            { data.length === 0 ? (
              <div className="text-center py-16 bg-white">
                ...
                <h3 className="text-primary-950 text-lg font-medium mb-1">No service providers found</h3>
                <p className="text-primary-950/70 mb-6">Add a new service provider to get started</p>
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-primary-950/5 py-5">
                    <tr className="text-left text-sm font-semibold text-primary-950 border-b border-primary-950/10">
                      ...
                      <th className="py-4 pl-3 text-left font-medium tracking-wider">Email</th>
                      <th className="py-4 pl-3 text-left font-medium tracking-wider">Mobile</th>
                      <th className="py-4 pl-3 text-left font-medium tracking-wider">Business</th>
                      <th className="py-4 pl-3 text-left font-medium tracking-wider">Category</th>
                      { showActions && <th className="py-4 pl-3 text-left font-medium tracking-wider">Actions</th> }
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-primary-950/10">
                    { data.map( ( entry, index ) => (
                      <tr key={ entry.id } onClick={ () => openModal( entry ) } className="hover:bg-primary-950/5 transition-colors">
                        ...
                        <td className="py-4 pl-3 font-medium text-primary-950">
                          <div className="w-40 truncate" title={ entry.name }>
                            { entry.name }
                          </div>
                        </td>
                        <td className="py-4 pl-3 text-primary-950">
                          <div className="w-40 truncate" title={ entry.email }>
                            { entry.email }
                          </div>
                        </td>
                        <td className="py-4 pl-3 text-primary-950">
                        ...
                        <td className="py-4 pl-3 text-primary-950">
                          <div className="w-40 truncate" title={ entry.business_name }>
                            { entry.business_name }
                          </div>
                        </td>
                        <td className="py-4 pl-3 text-primary-950">
                          <div className="w-40 truncate" title={ entry.categorie_name }>
                            { entry.categorie_name || 'N/A' }
                          </div>
                        </td>
                        { showActions && (
                          <td className="py-4 pl-3">
                            <div className="flex gap-2">
                              ...
                              <button
                                ...
                                className="px-3 mr-8 py-1.5 bg-white text-red-600 text-xs rounded-lg hover:bg-red-50 border border-red-200 shadow-sm hover:shadow transition-all flex items-center gap-1"
                                title="Delete"
                              >
      ...
      { isModalOpen && selectedProvider && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
            ...
            {/* Header */ }
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Service Provider Details</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-gray-800 font-medium">{ selectedProvider.name }</p>
                </div>

                <div>
                  <p className="text-sm font-medium text-gray-500">Email</p>
                  <p className="text-gray-800 font-medium">{ selectedProvider.email }</p>
                </div>

                <div>
                  <p className="text-sm font-medium text-gray-500">Mobile</p>
                  <p className="text-gray-800 font-medium">{ selectedProvider.mobile }</p>
                </div>

                <div>
                  <p className="text-sm font-medium text-gray-500">Business Name</p>
                  <p className="text-gray-800 font-medium">{ selectedProvider.business_name }</p>
                </div>

                <div>
                  <p className="text-sm font-medium text-gray-500">Category</p>
                  <p className="text-gray-800 font-medium">{ selectedProvider.categorie_name || 'N/A' }</p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="w-full">
                  ...
                  <p className="text-gray-800 font-medium">{ selectedProvider.address }</p>
                </div>
              </div>
            </div>
