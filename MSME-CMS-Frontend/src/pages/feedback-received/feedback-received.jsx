...
import { getContactList, getfeedbackList } from '../../api/user-lists';
import { exportFeedbackReceivedToExcel } from '../../utils/exports/excel-feedback-received-export';
...
const FeedbackReceived = () => {
  ...
  const fetchfeedback = async ( page, limit ) => {
    try {
      const data = await getfeedbackList( page, limit );
  ...
  return (
    <div className="p-6 px-10 min-h-screen">
      <div className="text-sm text-gray-500 pb-5">
        Pages / <span className="text-gray-800">Feedback Received</span>
      </div>
        ...
        <table className="min-w-full text-sm ">
          <thead className="bg-primary-950/5 py-5">
            <tr className="text-left text-sm font-semibold text-primary-950 border-b border-primary-950/10">
              ...
              <th className="py-3 pl-3  text-left font-medium  tracking-wider">Feedback Type</th>
            ...
            { feedbackData.length > 0 ? (
              feedbackData.map( ( feedback, index ) => (
                <tr key={ index } className="hover:bg-gray-50 cursor-pointer transition-colors" onClick={ () => openModal( feedback ) }>
                  ...
                  <td className="py-3 px-4  ">{ feedback.feedbackType }</td>
                  <td className="py-3 px-4  ">
                    { getFirstWords( feedback.message, 5 ) }
                  </td>
                </tr>
              ) )
            ) : (
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-800 font-medium capitalize">{ selectedFeedback.feedbackType }</p>
              </div>
            </div>
